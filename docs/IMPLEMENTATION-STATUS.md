...
Last audited: **2026-08-23**, against `main` @ `e84dcef`.
...
| 2a | Receive lead via `leadgen` webhook | ❌ | Payload parses, yields zero events, returns 200. **Lead is silently discarded.** |
| 2b | Create / update contact + lead records | 🟡 | Repositories exist and are tested; unreachable from a form submission. |
...
| **D-3** | **`leadgen` payloads are silently discarded.** `extractEvents` iterates only `value.messages` / `value.statuses`. A leadgen change parses cleanly, produces no events, and is ACKed 200. `envelope.object` and `change.field` are never inspected. | `src/whatsapp/payload.ts` | Enabling the `leadgen` webhook field today loses paid leads with no error. |
...
|---|---|---|---|
| **0** | Alignment documents (this set) | — | 🔄 In progress |
| **1** | `leadgen` intake: parse, retrieve by `leadgen_id`, persist contact + referral. **Sends nothing.** | 0 | Not started |
| **2** | Wire the consent gate (**D-1**) and window enforcement (**D-2**) | 1 | Not started |
...
Phase 1 requires **no migration** — `campaign_referrals.form_id` and
`.external_lead_id` already exist with a unique index.
...
| E-5 | `leads_retrieval` permission + Page access token + Page subscribed to `leadgen` | Phase 1 go-live |
...
377 tests across 28 files, colocated, with integration tests running against a
real PostgreSQL.
Coverage of the inbound engine is genuinely strong.

**Gap:** there is currently **no test that exercises the required end-to-end flow**.
Every test begins from an inbound message, so the entire proactive product could
remain unbuilt and the suite would stay green. Closing this is a Phase 1
deliverable — see [TRACEABILITY.md](TRACEABILITY.md).
