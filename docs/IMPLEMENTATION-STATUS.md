...
| ~~**D-14**~~ | **A time chosen in words looped.** D-11's fix left the words path to the classifier (`chosenOfferedTime`) and the writer's manners. Live (2026-09-23): "הכי מוקדם היום" was not resolved; the writer proposed 19:00 and asked "רוצה שאשריין?"; "כן" was not resolved; the writer asked again; "אישרתי כבר!!" — again; four times, until the person tapped the row. **Fixed in code, no model in the loop:** `resolveSlotChoice` reads the earliest/latest, an hour, an ordinal, a day, a part of the day, narrowed to today/tomorrow (never a question or a no); a short yes books the one time the bot's own last message talked about (`slotMentionedIn`); and after two written replies without a pick the third is the list and a plain ask to tap (`ASSIST_BOOKING_MAX`), with the list re-sent from the second reply on. | `appointments/slotMessages.ts` · `workflow/conversationTurn.ts` | Resolved. |
