...
| 4 | After screening, a bounded discovery conversation (≤ 3 questions) learns the property, the reason and what matters; skipped for a lead who asked for a meeting, cut to one for a terse ready-now lead | `workflow/decide.ts` (`nextScreeningStep`, `DISCOVERY_MAX`, `isTerseAnswer`, `discoveryContext`) · `workflow/generate.ts` (`ask_intent`) | `decide.test.ts` (intent check block: thin answer, both known, cap, meeting asked, terse vs talkative, no nagging) | ✅ |
