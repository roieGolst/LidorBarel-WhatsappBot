...
| 5 | The calendar event is titled with the person's name and its description carries the note (name, phone, answers, score, property details), on booking, move and reminder | `appointments/meetingNote.ts` · `appointments/booking.ts` · `appointments/exclusivityCallback.ts` (`תיאור חופשי` = `text_mm7fdakx`) | `meetingNote.test.ts` · `booking.test.ts` (name, fallback, note, rename on move) · `exclusivityCallback.test.ts` | ✅ |
