...
| 5 | The calendar event is titled with the person's name; the item carries a meeting note (name, phone, answers, score, property details) | `appointments/meetingNote.ts` · `appointments/booking.ts` · `appointments/exclusivityCallback.ts` · `monday/client.ts` (`createUpdate`) | `meetingNote.test.ts` · `booking.test.ts` (name, fallback, note, note failure, rename on move) | ✅ (event description: E-15) |
