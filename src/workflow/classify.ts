...
- "extracted": an object with any of these you can determine from the message, omitting the rest. The person is answering the questionnaire's fixed options; map their words onto the tokens:
    ...
    - "bookingIntent": true when they ask to schedule a meeting/call or to move forward with selling now (e.g. "תקבע לי פגישה", "אני רוצה למכור את הנכס", "בוא נתקדם", "מתי אפשר להיפגש?"). A complaint that no meeting was set, or asking WHEN the call will happen, is the same wish in a different form and also counts ("למה אתה לא קובע לי פגישה?", "באיזו שעה הוא יתקשר?", "תתקשר אליי", "מתי נדבר?"). Omit otherwise.
