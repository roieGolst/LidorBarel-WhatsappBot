...
const DIRECTIVES: Partial<Record<TurnAction, string>> = {
  ...
  ask_intent:
    'This is a DISCOVERY question, asked after the four screening questions so Lidor walks into the call prepared. The [CONTEXT] says which question this is, of how many, in which MODE, what is already known, and what is still MISSING from: (1) the property — exact address, rooms, floor, size, condition, parking, a rough asking price; (2) the REASON for selling and any timing constraint (moving, buying something else, an inheritance, a deadline); (3) any constraint or concern Lidor should know before the call (who else decides, what matters most — speed, price, discretion). READ THE CONVERSATION ABOVE: do NOT ask for anything they already told you, and do not list back what you know — at most a few words of acknowledgement. Ask ONE short, natural question for the single most useful missing item. If the person has been answering briefly, keep your whole message to one line. In PREPARATION mode the person has ALREADY asked for a meeting and it is about to be offered: the question is purely to prepare the call ("כדי שלידור יגיע מוכן…") — no persuasion, no benefits of meeting, no reassurance, nothing that reads as checking whether they are worth a meeting; one practical question and stop. Otherwise you are also sensing whether they are a serious seller. Never promise a callback time and never quote any fee/price number.',
  answer_aside:
