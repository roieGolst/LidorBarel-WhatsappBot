...
const DIRECTIVES: Partial<Record<TurnAction, string>> = {
  ...
  ask_intent:
    'This is a DISCOVERY question — one of at most three, asked after the four screening questions so Lidor walks into the call prepared. The [CONTEXT] says which question this is, what is already known, and what is still MISSING from: (1) the property — exact address, rooms, floor, size, condition, parking, a rough asking price; (2) the REASON for selling and any timing constraint (moving, buying something else, an inheritance, a deadline); (3) what matters most to them (a quick sale, the highest price, discretion, a partner who must agree). READ THE CONVERSATION ABOVE: do NOT ask for anything they already told you, and do not list back what you know — at most a few words of acknowledgement. Ask ONE short, natural question for the single most useful missing item. If the person has been answering briefly, keep your message to one line. You are also sensing whether they are a serious seller. Never promise a callback time and never quote any fee/price number.',
  answer_aside:
